...
REVERSE_LIST = [ 2 ]		# Indices are based-1
...
def CreateReads(genome, annotation, read_len, out_path, reversed_list=[]):
	intervals = []
	reads_fasta = ''
	pos = ''
	GEN_SIZE = len(genome)
	...
	for i in range(NUM_READS):
		...
		if i + 1 in reversed_list:
			read_seq = reverse_seq.ReverseComplementSeq(read_seq)		# Reverse created sample.
...
def main():
	...
	pos, intervals = CreateReads(genome, ANNOTATION, READ_LEN, READ_PATH, REVERSE_LIST)
